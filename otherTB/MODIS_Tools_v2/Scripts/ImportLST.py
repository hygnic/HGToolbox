...
try:
    ...
    dd = d[1].rjust(2,'0')                     #day
    yr = d[2]                                  #year
except IndexError:
    arcpy.AddMessage("\n[ERROR] Date must be in format: MM/DD/YYYY\n")
    sys.exit(0)

f = date(int(yr),int(m), int(dd))
...
def DownloadRoutine(tile):
    """Downloads files"""
    hdf_pattern = re.compile(sat_prefix + '11A1.A'+yr+DOY+'.'+ tile +'.006.*.hdf$', re.IGNORECASE)
...
source = 'https://e4ftl01.cr.usgs.gov/{0}/{1}11A1.006/{2}.{3}.{4}/'.format(sat_folder, sat_prefix, yr ,m, dd)
